### Hi 👋, I'm Richard Lippmann

**I am an 18 year old developer from Germany**

- 🌱 I’m currently learning **C++ and React**
- 🔎 I'm interested in **Programming, Servers, Game Development**!
- 📫 If you want to talk to me, reach out to me on: 
  - Discord: *@richardlippmann
  - Email: *richard.v.lippmann@gmail.com*

### Operating Systems
![Operating Systems](https://skillicons.dev/icons?i=arch,windows)

### Tools
![Tools](https://skillicons.dev/icons?i=git,github,githubactions,cloudflare,docker,mysql,npm,nodejs,bun,nginx,vscode,bash,d3,discord&perline=10)

### Libraries
![Libraries](https://skillicons.dev/icons?i=express,react,tailwind)

### Programming Languages
![Programming Languages](https://skillicons.dev/icons?i=js,java,py,css,html,cs,cpp)
